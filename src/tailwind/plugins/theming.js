...
const generateVariableColors = (theme) => {
  ...
  const customPropertiesWithOpacity = (name) => ({
    ...
    if (opacityValue) {
      return `rgba(var(--verb-${name}-rgb), ${opacityValue})`;
    }
    if (opacityVariable) {
      return `rgba(var(--verb-${name}-rgb), var(${opacityVariable}, 1))`;
    }
    return `rgb(var(--verb-${name}-rgb))`;
  };
...
const theming = plugin.withOptions((options) => ({
    ...
    const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
      ...
      ))), (value, key) => [[`--verb-${e(key)}`, value], [`--verb-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
    ]));
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
      ...
      const background = theme(`verb.customProps.background.${colorScheme}`);
      const foreground = theme(`verb.customProps.foreground.${colorScheme}`);
      ...
      return {
        [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
          ...
          ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--verb-${e(key)}`, value], [`--verb-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
          ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--verb-${e(key)}`, value], [`--verb-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        }
      };
    });
